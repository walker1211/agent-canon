...
func TestParseAcceptsCompileCodexOut(t *testing.T) {
	root := t.TempDir()
	opts, err := Parse([]string{"compile", "codex", "--project", root, "--out", "preview"}, root, root)
	if err != nil {
		t.Fatalf("Parse returned error: %v", err)
	}

	if opts.Command != "compile" {
		t.Fatalf("Command = %q, want compile", opts.Command)
	}
	if opts.CompileTarget != "codex" {
		t.Fatalf("CompileTarget = %q, want codex", opts.CompileTarget)
	}
	if opts.Out != "preview" {
		t.Fatalf("Out = %q, want preview", opts.Out)
	}
...
func TestParseAcceptsCompileClaudeOut(t *testing.T) {
	root := t.TempDir()
	opts, err := Parse([]string{"compile", "claude", "--project", root, "--out", "preview"}, root, root)
	if err != nil {
		t.Fatalf("Parse returned error: %v", err)
	}
	...
	if opts.CompileTarget != "claude" {
		t.Fatalf("CompileTarget = %q, want claude", opts.CompileTarget)
	}
	if opts.Out != "preview" {
		t.Fatalf("Out = %q, want preview", opts.Out)
	}
...
func TestParseRejectsInvalidCompileForms(t *testing.T) {
	root := t.TempDir()
	for _, tc := range []struct {
		name string
		args []string
	}{
		{name: "missing target", args: []string{"compile", "--project", root, "--out", "preview"}},
		{name: "claude extra arg", args: []string{"compile", "claude", "extra", "--project", root, "--out", "preview"}},
		{name: "unsupported target", args: []string{"compile", "other", "--project", root, "--out", "preview"}},
		{name: "extra arg", args: []string{"compile", "codex", "extra", "--project", root, "--out", "preview"}},
		{name: "codex missing out", args: []string{"compile", "codex", "--project", root}},
		{name: "claude missing out", args: []string{"compile", "claude", "--project", root}},
		{name: "claude json format", args: []string{"compile", "claude", "--project", root, "--format", "json", "--out", "preview"}},
		{name: "claude text format", args: []string{"compile", "claude", "--project", root, "--format", "text", "--out", "preview"}},
		{name: "include memory", args: []string{"compile", "claude", "--include-memory", "--project", root, "--out", "preview"}},
		{name: "dry run", args: []string{"compile", "claude", "--dry-run", "--project", root, "--out", "preview"}},
		{name: "yes", args: []string{"compile", "claude", "--yes", "--project", root, "--out", "preview"}},
		{name: "global", args: []string{"compile", "claude", "--global", "--project", root, "--out", "preview"}},
		{name: "ours", args: []string{"compile", "claude", "--ours", "--project", root, "--out", "preview"}},
		{name: "manual", args: []string{"compile", "claude", "--manual", "merged value", "--project", root, "--out", "preview"}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			_, err := Parse(tc.args, root, root)
			if err == nil {
