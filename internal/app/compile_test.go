...
func TestRunCompileClaudeRequiresCanonBaseline(t *testing.T) {
	fixture := copiedFixture(t, "basic")
	outDir := filepath.Join(t.TempDir(), "compiled")
	var stdout, stderr bytes.Buffer
	...
	if !strings.Contains(stderr.String(), "compile claude requires canon baseline") || !strings.Contains(stderr.String(), "run \"agent-canon sync claude codex\" first") {
		t.Fatalf("stderr missing baseline guidance: %q", stderr.String())
	}
	assertPathMissing(t, outDir)
}

func TestRunCompileClaudeRequiresSyncState(t *testing.T) {
	fixture := copiedFixture(t, "basic")
	runInitialSync(t, fixture)
	if err := os.Remove(filepath.Join(fixture.project, ".agent-canon", "sync-state.json")); err != nil {
		t.Fatalf("remove sync state: %v", err)
	...
	if !strings.Contains(stderr.String(), "compile claude requires sync state") || !strings.Contains(stderr.String(), "run \"agent-canon sync claude codex\" first") {
		t.Fatalf("stderr missing sync guidance: %q", stderr.String())
	}
	assertPathMissing(t, outDir)
}

func TestRunCompileClaudeBlocksOpenConflicts(t *testing.T) {
	fixture := copiedFixture(t, "basic")
	claudePath := filepath.Join(fixture.project, "CLAUDE.md")
	codexPath := filepath.Join(fixture.project, "AGENTS.md")
	writeFile(t, claudePath, "shared base\n")
	...
	if !strings.Contains(stderr.String(), "compile claude blocked") || !strings.Contains(stderr.String(), "open conflicts") || !strings.Contains(stderr.String(), "agent-canon conflicts") || !strings.Contains(stderr.String(), "agent-canon resolve") {
		t.Fatalf("stderr missing conflict guidance: %q", stderr.String())
	}
	assertPathMissing(t, outDir)
}

func TestRunCompileClaudeRejectsOutputInsideClaudeOrCodexHome(t *testing.T) {
	cases := []struct {
		name       string
		out        func(fixturePaths) string
		writeCheck func(fixturePaths) string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			code := app.Run([]string{"compile", "claude", "--project", fixture.project, "--claude-home", fixture.claudeHome, "--codex-home", fixture.codexHome, "--out", tc.out(fixture)}, fixture.project, fixture.home, &stdout, &stderr)
			if code != 1 {
				t.Fatalf("exit code = %d, want 1; stdout=%q stderr=%q", code, stdout.String(), stderr.String())
			}
			if !strings.Contains(stderr.String(), "Claude or Codex home") {
...
func TestRunCompileClaudeWritesPreviewAndDoesNotModifyInputs(t *testing.T) {
	fixture := copiedFixture(t, "basic")
	runInitialSync(t, fixture)
	workspaceRoot := filepath.Join(fixture.project, ".agent-canon")
	canonPath := filepath.Join(workspaceRoot, "base", "canon.snapshot.json")
...
func TestRunCompileClaudeDoesNotLeakSecrets(t *testing.T) {
	fixture := copiedFixture(t, "secrets")
	const secret = "ghp_agent_canon_fixture_secret_must_not_leak"
	runInitialSync(t, fixture)
	outDir := filepath.Join(t.TempDir(), "compiled")
